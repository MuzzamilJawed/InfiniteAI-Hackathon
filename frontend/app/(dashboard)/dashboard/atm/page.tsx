      ...
      >
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold">SafeBank ATM</span>
          </div>
        ...
        <div className="mt-6 rounded-2xl bg-slate-900/70 p-5 border border-white/10">
          ...
          <div className="mt-2 flex gap-2">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className={`size-10 rounded-lg grid place-items-center font-semibold ${
                  pin.length > i ? "bg-[var(--gold)]/20" : "bg-white/5"
                }`}
              >
                {pin[i] ? "•" : ""}
              </div>
