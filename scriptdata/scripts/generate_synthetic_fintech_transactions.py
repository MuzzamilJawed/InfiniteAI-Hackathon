...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--include-label", action="store_true")
    return p.parse_args()
...
def make_profile(uid: int) -> UserProfile:
    ...
    return UserProfile(uid, cohort, float(clip(avg_amount, 8, 6000)), float(volatility), float(base_velocity), int(history), float(trusted), float(city), float(ben))
...
def normal_tx(p: UserProfile, idx: int, total: int) -> dict:
    ...
    device = "trusted_device" if random.random() < p.trusted_device_prob else ("new_device" if random.random() < 0.82 else "emulated_device")
    location = "same_city" if random.random() < p.same_city_prob else "different_city"
    return {
        "user_id": p.user_id,
...
def inject_anomalies(df: pd.DataFrame, ratio: float) -> pd.DataFrame:
    n = max(1, int(round(len(df) * ratio)))
    idxs = np.random.choice(np.arange(len(df)), size=n, replace=False)
    df.loc[idxs, "is_anomaly"] = 1
    drift_users = set(np.random.choice(df["user_id"].unique(), size=max(1, int(df["user_id"].nunique() * 0.06)), replace=False))
    for i in idxs:
        r = df.loc[i].copy()
        r["amount"] = round(clip(float(r["avg_user_amount"]) * np.random.uniform(3.0, 10.0), 20.0, 25000.0), 2)
        r["tx_velocity"] = int(max(float(r["tx_velocity"]), round(np.random.randint(8, 40) * (1.0 + min(float(r["tx_history"]) / 1200.0, 1.5)))))
        ...
        if int(r["user_id"]) in drift_users and random.random() < 0.75:
            r["avg_user_amount"] = round(clip(float(r["avg_user_amount"]) * np.random.uniform(1.8, 3.8), 6.0, 9000.0), 2)
...
def summary(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "corr_amount_avg_user_amount": corr(df["amount"], df["avg_user_amount"]),
...
def write_md(s: dict, out: Path) -> None:
    lines = [
        ...
        f"- Correlation (`amount`, `avg_user_amount`): **{s['corr_amount_avg_user_amount']:.3f}**",
        ...
        f"- Location distribution: **{json.dumps(s['location_distribution'])}**",
    ]
...
def main() -> None:
    ...
    for p, c in zip(profiles, counts):
        for i in range(int(c)):
            rows.append(normal_tx(p, i, int(c)))

    df = pd.DataFrame(rows)
    df = inject_anomalies(df, a.anomaly_ratio)
    ...
    export_cols = ["amount", "avg_user_amount", "amount_deviation", "tx_velocity", "tx_history", "new_beneficiary", "device", "location"]
    if a.include_label:
    ...
    print(f"Correlation(amount, avg_user_amount): {s['corr_amount_avg_user_amount']:.3f}")
